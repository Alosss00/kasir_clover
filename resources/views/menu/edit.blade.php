<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-slate-900">Edit Menu: {{ $menu->nama_menu }}</h2>
                <p class="text-xs text-slate-500 mt-0.5">Ubah takaran resep atau harga jual menu</p>
            </div>
    ...
    <div 
        x-data="{
            namaMenu: {{ Js::from($menu->nama_menu) }},
            kategori: {{ Js::from($menu->kategori) }},
            hargaJual: {{ (float)$menu->harga_jual }},
            biayaLain: {{ (float)$menu->biaya_lain }},
            bahanOptions: {{ Js::from($bahanBakuList) }},
            resepRows: {{ Js::from($menu->resep->map(fn($r) => ['bahan_baku_id' => $r->bahan_baku_id, 'jumlah_pemakaian' => (float)$r->jumlah_pemakaian])) }},
            init() {
                if (this.resepRows.length === 0) {
                    this.addRow();
                }
            },
            addRow() {
